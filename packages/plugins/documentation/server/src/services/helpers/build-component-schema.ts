...
const getRequiredAttributes = (allAttributes: Schema.Attributes) => {
...
const getAllSchemasForContentType = ({ routeInfo, attributes, uniqueName }: ApiInfo) => {
  ...
  const routeMethods = routeInfo.routes.map((route: Common.Route) => route.method);
  ...
  const hasListOfEntities = routeInfo.routes.filter((route: Common.Route) =>
